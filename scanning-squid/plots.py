...
class TDCPlot(object):
    ...
    def update(self, data_set: Any, counter: Any, getting_plane: bool) -> None:
        """Update plot with data from data_set.

        Args:
            data_set: DataSet generated by Loop in Microscope.td_cap().
            counter: utils.Counter instance to know where we are in the Loop.
            getting_plane: True if Microscope.td_cap() is being called in a loop
                to acquire a plane.
        """
        self.location = data_set.location
        nwindow = self.constants['N_window']
        pt = counter.count
        self.data = self._to_real_units(data_set)
        self.meta = data_set.metadata['loop']['metadata']
        if pt < len(self.heights):
